...
with open('model_For_Lactamase_Without_2_descriptors.pkl','rb') as b:
        model_Lac = pickle.load(b)
with open('Newest_Model_Shoichet&ZINC.pkl','rb') as c:
        model_Shoichet = pickle.load(c)      
...
def compute_similarity_and_AD(df_train, descriptor_columns, target_col, test_data):
    # Separate binary and continuous columns
    binary_cols = [col for col in descriptor_columns if 'Col' in col]
    continuous_cols = [col for col in descriptor_columns if col not in binary_cols]
    
    # Balance the training data
    X = df_train[descriptor_columns]
    ...
    X_continuous = X_resampled[continuous_cols].values
    X_continuous_scaled = scaler.fit_transform(X_continuous)
    X_scaled = np.concatenate((X_continuous_scaled, X_resampled[binary_cols].values), axis=1)
    
    # Preprocess and scale the test data
    X_test_continuous = test_data[continuous_cols].values
    X_test_continuous_scaled = scaler.transform(X_test_continuous)  # Use transform, not fit_transform
    X_test_scaled = np.concatenate((X_test_continuous_scaled, test_data[binary_cols].values), axis=1)
    
    # Compute similarity
    ad = ApplicabilityDomain(verbose=True)
    sims = ad.analyze_similarity(base_test=X_test_scaled[:, len(continuous_cols):], base_train=X_scaled[:, len(continuous_cols):], similarity_metric='tanimoto')
    
    # Assuming you've defined calculate_similarity_threshold and label_predictions functions here...
    Dc = calculate_similarity_threshold(X_scaled[:, :len(continuous_cols)], k=5, Z=0.5)
    predictions = label_predictions(X_scaled[:, :len(continuous_cols)], X_test_continuous_scaled, k=5, Dc=Dc)
    
    # Combine results
    sims = sims.reset_index(drop=True)
    df_predictions = pd.DataFrame({'Index': range(len(predictions)), 'Prediction': predictions}).reset_index(drop=True)
    sims['Continous_Reliability'] = df_predictions['Prediction']
    
    # Categorize results
    conditions = [
        (sims['Max'] >= 0.8) & (sims['Continous_Reliability'] == 'Reliable'),
        (sims['Max'] >= 0.8) & (sims['Continous_Reliability'] == 'Unreliable'),
        (sims['Max'] < 0.8) & (sims['Continous_Reliability'] == 'Reliable'),
        (sims['Continous_Reliability'] == 'Unreliable')
    ]
    values = ['Absolute reliable', 'Structurally reliable', 'Distance reliable', 'Unreliable']
    sims['Ultimate_reliability'] = pd.Series(np.select(conditions, values))
...
if one_or_few_SMILES is not None:
    try:
        ...
        # Check if the provided input is a valid list of SMILES notations
