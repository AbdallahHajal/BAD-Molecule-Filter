...
with open('x_train1_cruzain.pkl', 'rb') as file:
    x_train1_cruzain = pickle.load(file)
with open('x_train1_Lac.pkl', 'rb') as file:
    x_train1_Lac = pickle.load(file)
with open('x_train1_Shoich.pkl', 'rb') as file:
    x_train1_Shoichet = pickle.load(file)
...
binary_cols_CRU = [col for col in descriptor_columns_cru if 'Col' in col]
continuous_cols_CRU = [col for col in descriptor_columns_cru if col not in binary_cols_CRU]
...
binary_cols_Lac = [col for col in descriptor_columns_Lac if 'Col' in col]
continuous_cols_Lac = [col for col in descriptor_columns_Lac if col not in binary_cols_Lac]
start_column_Lac= 252
end_column_Lac = 252

# For Shoichet Start_column=284 & end=column= 284 Now its 287 after adding zinc
# For Cruzain Start_column=178 (163) & end=column= 178 (163)
# For Lactamase Start_column=291 (252) & end=column= 291 (252)
...
binary_cols_Shoi = [col for col in descriptor_columns_Shoi if 'Col' in col]
continuous_cols_Shoi = [col for col in descriptor_columns_Shoi if col not in binary_cols_Shoi]
...
def ad_strategy():
    ...
    # Styling the header for instructions
    st.sidebar.markdown("### Instructions to obtain SMILES from Ketcher:")

    # Individual instructions with emphasis on key steps
    st.sidebar.markdown("1. Draw your molecule in the **Ketcher editor**.")
    st.sidebar.markdown("2. Press on the **Save As** option in Ketcher.")
    st.sidebar.markdown("3. Select **SMILES** from the dropdown menu.")
    st.sidebar.markdown("4. **Copy** the provided SMILES representation.")
    ...
    if one_or_few_SMILES != "['CCCCO']":
        ...
        start_column = 163
        x_train1_binary_dataframe = pd.DataFrame(x_train1_cruzain[:, start_column:], columns=binary_cols_CRU)
        x_train1_b_array = x_train1_binary_dataframe.values
        start_column = 163

        x_test2_dataframe = pd.DataFrame(X_test_scaled_Cru[:, start_column:], columns=binary_cols_CRU)
        x_test2_array = x_test2_dataframe.values

        import pandas as pd
        import numpy as np
        from pyADA import ApplicabilityDomain
        ad = ApplicabilityDomain(verbose=True)
        sims = ad.analyze_similarity(base_test=x_test2_array, base_train=x_train1_b_array,
                                     similarity_metric='tanimoto')
        sims

        k = 5  # Number of nearest neighbors
        Z = 0.5  # Significance level

        end_column = 163
        x_test2_continuous = pd.DataFrame(X_test_Cruzain[:, :end_column], columns=continuous_cols_CRU)
        # Convert x_train1_binary to an array
        x_test2_c_array = x_test2_continuous.values

        end_column = 163
        x_train1_continuous = pd.DataFrame(X_test_scaled_Cru[:, :end_column], columns=continuous_cols_CRU)

        # Convert x_train1_binary to an array
        x_train1_c_array = x_train1_continuous.values


        #Xi=preprocessing.StandardScaler().fit(Xi).transform(Xi.astype(float))


        Dc = calculate_similarity_threshold(x_train1_c_array, k, Z)

        predictions = label_predictions(x_train1_c_array, x_test2_c_array, k, Dc)

        # Create a DataFrame to store the predictions

        #Index starts from 1
        #df_predictions = pd.DataFrame({'Index': range(1, len(predictions) + 1), 'Prediction': predictions})

        #Index starts from 0
        df_predictions = pd.DataFrame({'Index': range(len(predictions)), 'Prediction': predictions})

        #Here i added from Similarity by PyADA and from distance together
        # Assuming your original DataFrame is called 'df'
        sims_Cruzain = sims.reset_index(drop=True)
        df_predictions_Shoichet_Cru = df_predictions.reset_index(drop=True)
        sims_Cruzain['Continous_Reliability'] = df_predictions_Shoichet_Cru['Prediction']


        # Define the conditions
        conditions = [
            (sims_Cruzain['Max'] >= 0.8) & (sims_Cruzain['Continous_Reliability'] == 'Reliable'),                      # Absolute reliable
            (sims_Cruzain['Max'] >= 0.8) & (sims_Cruzain['Continous_Reliability'] == 'Unreliable'),                    # Structurally reliable
            (sims_Cruzain['Max'] < 0.8) & (sims_Cruzain['Continous_Reliability'] == 'Reliable'),                      # Distance reliable
            (sims_Cruzain['Continous_Reliability'] == 'Unreliable')                                        # Unreliable
        ]

        # Define the corresponding values for each condition
        values = ['Absolute reliable', 'Structurally reliable', 'Distance reliable', 'Unreliable']

        # Create the 'Ultimate_reliability' column based on the conditions and values
        sims_Cruzain['Ultimate_reliability'] = pd.Series(np.select(conditions, values))
        ...
        start_column = 252
        x_train1_binary_dataframe = pd.DataFrame(x_train1_Lac[:, start_column:], columns=binary_cols_Lac)
        x_train1_b_array = x_train1_binary_dataframe.values
        start_column = 252
        x_test2_dataframe = pd.DataFrame(X_test_scaled_Lac[:, start_column:], columns=binary_cols_Lac)
        x_test2_array = x_test2_dataframe.values
        ...
        end_column = 252
        x_test2_continuous = pd.DataFrame(X_test_scaled_Lac[:, :end_column], columns=continuous_cols_Lac)
        # Convert x_train1_binary to an array
        x_test2_c_array = x_test2_continuous.values

        end_column = 252
        x_train1_continuous = pd.DataFrame(x_train1_Lac[:, :end_column], columns=continuous_cols_Lac)
        ...
        #Here i added from Similarity by PyADA and from distance together
        # Assuming your original DataFrame is called 'df'
        sims_Cruzain_lac = sims.reset_index(drop=True)
        df_predictions_Shoichet_lac = df_predictions.reset_index(drop=True)
        sims_Cruzain_lac['Continous_Reliability'] = df_predictions_Shoichet_lac['Prediction']


        # Define the conditions
        conditions = [
            (sims_Cruzain_lac['Max'] >= 0.8) & (sims_Cruzain_lac['Continous_Reliability'] == 'Reliable'),                      # Absolute reliable
            (sims_Cruzain_lac['Max'] >= 0.8) & (sims_Cruzain_lac['Continous_Reliability'] == 'Unreliable'),                    # Structurally reliable
            (sims_Cruzain_lac['Max'] < 0.8) & (sims_Cruzain_lac['Continous_Reliability'] == 'Reliable'),                      # Distance reliable
            (sims_Cruzain_lac['Continous_Reliability'] == 'Unreliable')                                        # Unreliable
        ]
        ...
        # Create the 'Ultimate_reliability' column based on the conditions and values
        sims_Cruzain_lac['Ultimate_reliability'] = pd.Series(np.select(conditions, values))
        ...
        start_column = 287
        x_train1_binary_dataframe = pd.DataFrame(x_train1_Shoichet[:, start_column:], columns=binary_cols_Shoi)
        x_train1_b_array = x_train1_binary_dataframe.values
        x_test2_dataframe = pd.DataFrame(X_test_scaled_Shoi[:, start_column:], columns=binary_cols_Shoi)
        x_test2_array = x_test2_dataframe.values
        ...
        sims = ad.analyze_similarity(base_test=x_test2_array, base_train=x_train1_b_array,
                                     similarity_metric='tanimoto')
        sims
        #sims.to_csv('Tanimoto_Similatiry_pyADA_Shoichet.csv', index=False)
        ...
        # Example usage
        # Assuming you have your training features (train_features) and test features (test_features) as numpy arrays
        import pandas as pd
        ...
        end_column = 287
        x_test2_continuous = pd.DataFrame(X_test_scaled_Shoi[:, :end_column], columns=continuous_cols_Shoi)
        # Convert x_train1_binary to an array
        x_test2_c_array = x_test2_continuous.values

        end_column = 287
        x_train1_continuous = pd.DataFrame(x_train1_Shoichet[:, :end_column], columns=continuous_cols_Shoi)
        ...
        #Here i added from Similarity by PyADA and from distance together
        # Assuming your original DataFrame is called 'df'
        sims_Cruzain_shoi = sims.reset_index(drop=True)
        df_predictions_Shoichet = df_predictions.reset_index(drop=True)
        sims_Cruzain_shoi['Continous_Reliability'] = df_predictions_Shoichet['Prediction']


        # Define the conditions
        conditions = [
            (sims_Cruzain_shoi['Max'] >= 0.8) & (sims_Cruzain_shoi['Continous_Reliability'] == 'Reliable'),                      # Absolute reliable
            (sims_Cruzain_shoi['Max'] >= 0.8) & (sims_Cruzain_shoi['Continous_Reliability'] == 'Unreliable'),                    # Structurally reliable
            (sims_Cruzain_shoi['Max'] < 0.8) & (sims_Cruzain_shoi['Continous_Reliability'] == 'Reliable'),                      # Distance reliable
            (sims_Cruzain_shoi['Continous_Reliability'] == 'Unreliable')                                        # Unreliable
        ]
        ...
        # Create the 'Ultimate_reliability' column based on the conditions and values
        sims_Cruzain_shoi['Ultimate_reliability'] = pd.Series(np.select(conditions, values))
        #sims.to_csv('TRIAIAI.csv', index=False)
        ...
        def majority_vote(predictions):
            return 1 if np.sum(predictions) >= 2 else 0
        
        dataframes = [sims_Cruzain, sims_Cruzain_lac, sims_Cruzain_shoi]
        indices_list = get_applicable_indices(dataframes)
        
        common_indices = list(set.intersection(*indices_list))

        # Filter test datasets
        X_test_scaled_Cru = X_test_scaled_Cru[common_indices]
        X_test_scaled_Shoi = X_test_scaled_Shoi[common_indices]
        X_test_scaled_Lac = X_test_scaled_Lac[common_indices]
        
        
        compound_predictions = []
        true_labels = []

        for i in range(len(X_test_scaled_Cru)):
            # Obtain the predicted probabilities for the current compound from each model
            model1_probs = model_Shoichet.predict_proba([X_test_scaled_Shoi[i]])[0]
            model2_probs = model_Lac.predict_proba([X_test_scaled_Lac[i]])[0]
            model3_probs = model_Cruz.predict_proba([X_test_scaled_Cru[i]])[0]

            # Calculate the average predicted probabilities across models
            avg_probs = (model1_probs + model2_probs + model3_probs) / 3.0

            # Predict the class with the highest average probability
            consensus_class = np.argmax(avg_probs)

            compound_predictions.append(consensus_class)
